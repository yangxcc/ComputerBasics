...
- 2021年11月18日，复习了操作系统的负数补码和内核等知识
  ...
  - Linux内核 V.S. Windows内核
    ...
    - 内核作为应用程序和硬件设备之间的桥梁，使得应用程序可以不用关注硬件设备的细节就可以执行，内核有很高的权限能够操作CPU、内存等硬件设备，所以操作系统被分成了**用户态和内核态**，程序切换到不同状态下需要使用**中断**
    - 内核的功能：进程管理、内存管理、硬件通信管理、提供系统调用
    - Linux内核的设计理念：宏内核，可执行文件格式ELF，MultiTask，SMP（对称任务处理，CPU地位一样）
    - Windows内核的设计理念：混合内核（微内核 + 宏内核），可执行文件格式PE，MultiTask，SMP（对称任务处理，CPU地位一样）
...
- 2021年11月19日，复习了计算机网络中的TCP四次挥手
  ...
  - 如果连接过程中有一个掉线了（应该是长时间没有动作发生），如果在线的一方开启了保活机制，那么会向掉线的一方发送探测报文  （2小时  75秒  9次）

    保活机制三种回应状态：一是客户端正常存活，那么会回复ACK报文，二是客户端掉线之后重新上线，但是由于丢失了连接信息，会回复RST报文中断连接；三是客户端持续掉线，这时需要走完整个保活机制全程
...
- 2021年11月20日，复习了网络中的Socket编程和操作系统中的内存管理机制
  ...
  - :blue_book:操作系统
    ...
    直接操作物理地址没有办法做到多个进程同时执行，所以就引入了虚拟地址空间，虚拟地址和物理地址的转换是通过MMU来完成的

    - 连续内存管理，把一个进程全部都给放到内存上运行，需要移动的太多了

    - 分段机制，把程序分成四个段：代码段、数据段、堆段、栈段，虚拟地址被分成了两个部分：段选择因子+段内偏移量，其中段选择因子中包括段号，特殊权限标记等，短号是最重要的，每个段的大小不固定

    - 分页机制，将虚拟地址和物理地址都化成固定大小的页，Linux中每页为4KB，虚拟地址分成了两个部分：页号 + 页内偏移量，每个进程都对应一个页表，为了解决空间问题，采取多级页表，为了解决多级页表中存在的时间问题（多了地址转换的过程），采用了TLB快表，把最常访问的页表项先放入到快表中

    - 段页式管理，每个程序对应一个段表，段表中的每一项对应一个页表，段表中存放的是页表的起始地址，页表中存放的是物理地址，段号 + 段内页号 + 页内偏移量

      > 分段内存管理会出现外部碎片，而分页内存管理会出现内部碎片，分段相当于对程序数据的分割，分页相当于对物理内存的分割，所以分页是没有任何的逻辑意义的，而分段是有意义的，比如数据段、代码段等
      >
      > 所以在分页中，一个Main函数可能会被分到多个页中，这多个页中有不使用的内存，这就造成了内部碎片，而分段机制中，这个Main函数都在一个段中，不会出现内存碎片

    Linux内存管理主要采用的是分页机制，但是由于Intel CPU发展的缘故，绕不开分段的思想，因此他将每段的地址都设置成了从0到4G（32位），这样每个程序都相当于面对的是线性地址

    在Linux操作系统中，虚拟地址空间又被分成了用户空间和内核空间，用户空间相互独立，但是内核空间对应的缺失同一块物理内存
  ...
  - :closed_book:网络
    ...
    - 基于TCP的Socket网络编程模型
      ...
      发起连接的一方（假设是客户端）调用connect，服务器需要依次调用bind，listen和accept
    ...
    - 基于UDP的Socket网络编程模型，仍然是需要bind，因为UDP通信也是需要IP地址和端口的
    
    - 本地的Socket通信，它既可以是基于字节流的，也可以是基于数据报的，但是和TCP/UDP最大的不同之处是他绑定的是一个本地socket文件
...
- 2021年11月21日，复习了操作系统中的进程和线程的基本概念以及模型；网络中的TCP重传机制和滑动窗口
  ...
  - :blue_book:操作系统
    ...
    - 进程，程序的一次动态执行
      ...
      - 进程发生切换的场景：时间片用完了、进程阻塞了（比如资源不够了）、进程被挂起了、有高优先级的进程到来了、发生中断了等，总之就是只要进程退出CPU就会发生切换
    ...
    - 线程，进程的一条执行流程，共享进程的虚拟地址、全局变量等资源，独享自己的寄存器、栈等资源
      ...
      - 线程的实现
        ...
        - 轻量级进程程LWP，内核空间支持的用户线程，由操作系统管理，和用户线程的对应模型为`1:1,1:N,M:N`
