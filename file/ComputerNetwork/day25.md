...
电信号的数据包到达网线接口时，交换机开始接收数据，将电信号转换成数字信号，然后校验网络包末尾的FCS，通过的话则把这个网络包放到自己的缓冲区中，后面的工作方式就和网卡的工作方式不同了，交换机不会检查网络包中的IP地址，因为**交换机的端口不具有IP地址**
...
- 交换机是基于以太网设计的，俗称二级网络设备。交换机的各个端口都没有IP地址



路由器的工作方式和交换机类似，都是收到网络包之后，先检查网络包末尾的帧校验序列，不同的是，路由器还需要检查网络包的IP地址，都通过校验之后才将网络包放入到自己的缓冲区中。
